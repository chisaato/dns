...
void setupLuaBindingsPacketCache(LuaContext& luaCtx, bool client)
{
  ...
  luaCtx.registerFunction<bool (std::shared_ptr<DNSDistPacketCache>::*)(const std::string&) const>("save", [](const std::shared_ptr<DNSDistPacketCache>& cache, const std::string& fname) {
    if (cache) {
      return cache->save(fname);
    }
    return false;
  });
  luaCtx.registerFunction<bool (std::shared_ptr<DNSDistPacketCache>::*)(const std::string&)>("load", [](std::shared_ptr<DNSDistPacketCache>& cache, const std::string& fname) {
    if (cache) {
      return cache->load(fname);
    }
    return false;
  });
  luaCtx.writeFunction("setPersistentCacheConfig", [](std::optional<LuaAssociativeTable<boost::variant<bool, size_t, std::string>>> vars) {
    setLuaSideEffect();
    if (!vars) return;
    dnsdist::configuration::updateRuntimeConfiguration([&vars](dnsdist::configuration::RuntimeConfiguration& config) {
      std::string dir;
      if (getOptionalValue<std::string>(vars, "directory", dir) > 0) {
        config.d_persistentCacheSettings.d_directory = dir;
      }
      size_t interval;
      if (getOptionalValue<size_t>(vars, "saveInterval", interval) > 0) {
        config.d_persistentCacheSettings.d_saveInterval = static_cast<uint32_t>(interval);
      }
      size_t minDirty;
      if (getOptionalValue<size_t>(vars, "minDirtyEntries", minDirty) > 0) {
        config.d_persistentCacheSettings.d_minDirtyEntries = static_cast<uint32_t>(minDirty);
      }
    });
  });
