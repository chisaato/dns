...
void setupLuaBindingsDNSQuestion([[maybe_unused]] LuaContext& luaCtx)
{
  ...
  luaCtx.registerFunction<void (DNSQuestion::*)(uint16_t code)>("removeEDNSOption", [](DNSQuestion& dnsQuestion, uint16_t code) {
    uint16_t optStart = 0;
    size_t optLen = 0;
    bool last = false;
    ...
    int res = removeEDNSOptionFromOPT(
      reinterpret_cast<char*>(&dnsQuestion.getMutableData().at(optStart)),
      &optLen,
      code
    );

    if (res == 0) {
      dnsQuestion.getMutableData().resize(
        dnsQuestion.getData().size() - (existingOptLen - optLen)
      );
    }
  });
